//
//  UILabel+SJYAttributeTextTapAction.m
//
...
@implementation UILabel (SJYAttributeTextTapAction)
...
- (id<SJYAttributeTapActionDelegate>)delegate
{
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setDelegate:(id<SJYAttributeTapActionDelegate>)delegate
{
    objc_setAssociatedObject(self, @selector(delegate), delegate, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

#pragma mark - mainFunction
- (void)sjy_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings tapClicked:(void (^) (NSString *string , NSRange range , NSInteger index))tapClick
{
    [self  sjy_getRangesWithStrings:strings];
...
- (void)sjy_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings
                                   delegate:(id <SJYAttributeTapActionDelegate> )delegate
{
    [self  sjy_getRangesWithStrings:strings];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [self  sjy_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        
        if (weakSelf.tapBlock) {
            weakSelf.tapBlock (string , range , index);
        }
        
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector( sjy_attributeTapReturnString:range:index:)]) {
            [weakSelf.delegate  sjy_attributeTapReturnString:string range:range index:index];
        }
        
        if (self.isTapEffect) {
            
            [self  sjy_saveEffectDicWithRange:range];
            
            [self  sjy_tapEffectWithStatus:YES];
        }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (self.isTapAction) {
        if ([self  sjy_getTapFrameWithTouchPoint:point result:nil]) {
            return self;
        }
    }
...
- (BOOL) sjy_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    CGAffineTransform transform = [self  sjy_transformForCoreText];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self  sjy_getLineBounds:line point:linePoint];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    if (self.isTapEffect) {
        ...
        [self performSelectorOnMainThread:@selector( sjy_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
...
- (CGAffineTransform) sjy_transformForCoreText
{
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);
}

- (CGRect) sjy_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    CGFloat ascent = 0.0f;
...
- (void) sjy_tapEffectWithStatus:(BOOL)status
{
    if (self.isTapEffect) {
...
- (void) sjy_saveEffectDicWithRange:(NSRange)range
{
...
- (void) sjy_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf  sjy_getStringWithRange:range]];
...
- (NSString *) sjy_getStringWithRange:(NSRange)range
{
