...
NSString * const cm_NoDataViewObserveKeyPath_collection = @"frame";
                                  ...
                                  offsetY:(CGFloat)offsetY
{
    ...
    //  实现跟随 TableView 滚动
    [view addObserver:self forKeyPath:cm_NoDataViewObserveKeyPath_collection options:NSKeyValueObservingOptionNew context:nil];
    return view;
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    if ([keyPath isEqualToString:cm_NoDataViewObserveKeyPath_collection]) {
...
- (void)removeObserverIfNeeded {
    if ([self.backgroundView isKindOfClass:[DXNoDataView class]]) {
        [self.backgroundView removeObserver:self forKeyPath:cm_NoDataViewObserveKeyPath_collection];
    }
}
...
-(void)setRefreshBlock:(CollectionViewRefreshBtnBlock)refreshBlock{
    objc_setAssociatedObject(self, @selector(refreshBlock), refreshBlock, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
-(CollectionViewRefreshBtnBlock)refreshBlock{
    return  objc_getAssociatedObject(self, _cmd);
}
@end
