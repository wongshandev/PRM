...
@end
@implementation XMKZDetialListCell
...
-(void)setupCell{
    ...
    contractMenLab.text = @"消费时间:";
    [self addSubview:contractMenLab];
    self.contractMenLab = contractMenLab;

    QMUILabel *contractLab = [self createLabelWithTextColor:Color_TEXT_NOMARL Font:Font_ListOtherTxt numberOfLines:1];
...
-(void)loadContent{
    XMKZDetialListModel *model = self.data;
    self.titleLab.text = model.titleStr;
    self.leftCircleLab.text = model.stateString;
     self.contractLab.text = model.OccurDate;
    self.reimburseLab.text = [NSString numberMoneyFormattor:model.Amount];
}
